...
### BUG-10. 720+ 구매 회차를 한 주 전으로 잘못 저장
- **증상**: state `lotto720.round=317` 인데 구매내역/실제 구매분은 318 (번호 605012 동일). state-ledger 회차 불일치로 `load_720(318)` 미스 → state 기반 번호 복원 불가.
- **원인**: `buy_lotto720` 가 `_extract_720_round()` 로 게임 페이지 "제 N 회" 텍스트를 읽는데, 이게 직전 추첨 결과 회차(317)를 잡아 한 주 밀림. 금요일 구매분의 실제 회차는 다음 목요일(318).
- **수정**: `_calc_purchase_720_round()` 추가(1회=2020-05-07 목, 오늘 이후 가장 가까운 목요일 회차). 이를 우선 사용하고 페이지 추출은 fallback/크로스체크로만. 검증: 2026-05-29(금)→318, 05-28(목)→317.
